...
import {ImpersonalItTranslationSentences} from '../../utils/translation';

const ImpersonalItTranslation = () => {
  const [totalQuestions, setTotalQuestions] = useState(0);
  const [correctAnswers, setCorrectAnswers] = useState(0);
  const [wrongAnswers, setWrongAnswers] = useState(0);
  const [isAnswerVisible, setIsAnswerVisible] = useState(true);
  const [selectedCells, setSelectedCells] = useState(['ImpersonalIt']);
  ...
  const generateQuestion = () => {
    ...
    let newTranslationSentences =
      ImpersonalItTranslationSentences[newSelectedSymbol];
  ...
  const checkAnswer = () => {
    ...
    if (textInputValue.join(' ') === answer) {
      ...
      const translationSentences =
        ImpersonalItTranslationSentences[selectedCell as SymbolKeys];
...
export default ImpersonalItTranslation;
