...
import {AAnTheTranslationSentences} from '../../utils/translation';

const AAnTheTranslation = () => {
  const [totalQuestions, setTotalQuestions] = useState(0);
  const [correctAnswers, setCorrectAnswers] = useState(0);
  const [wrongAnswers, setWrongAnswers] = useState(0);
  const [isAnswerVisible, setIsAnswerVisible] = useState(true);
  const [selectedCells, setSelectedCells] = useState(['AAn', 'The', 'All']);
  ...
  useEffect(() => {
    console.log(
      'AAnTheTranslationSentences',
      JSON.stringify(AAnTheTranslationSentences, null, 2),
    );
  }, []);
  ...
  const generateQuestion = () => {
    ...
    let newTranslationSentences = AAnTheTranslationSentences[newSelectedSymbol];
  ...
  const checkAnswer = () => {
    ...
    if (textInputValue.join(' ') === answer) {
      ...
      const translationSentences =
        AAnTheTranslationSentences[selectedCell as SymbolKeys];
...
export default AAnTheTranslation;
