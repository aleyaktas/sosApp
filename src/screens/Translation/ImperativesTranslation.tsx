...
import {ImperativesTranslationSentences} from '../../utils/translation';

const ImperativesTranslation = () => {
  const [totalQuestions, setTotalQuestions] = useState(0);
  const [correctAnswers, setCorrectAnswers] = useState(0);
  const [wrongAnswers, setWrongAnswers] = useState(0);
  const [isAnswerVisible, setIsAnswerVisible] = useState(true);
  const [selectedCells, setSelectedCells] = useState(['Imperatives']);
  ...
  const generateQuestion = () => {
    ...
    let newTranslationSentences =
      ImperativesTranslationSentences[newSelectedSymbol];
  ...
  const checkAnswer = () => {
    ...
    if (textInputValue.join(' ') === answer) {
      ...
      const translationSentences =
        ImperativesTranslationSentences[selectedCell as SymbolKeys];
      ...
      <ScrollView style={styles.scrollView}>
        <View
          ...
          <CellSelectionComponent
            ...
            cells={[{value: 'Imperatives', label: 'Emir Cümleleri'}]}
          />
          <QuestionComponent
            sentence={sentence}
...
export default ImperativesTranslation;
